...
- Área inferior adaptativa: se eliminó la doble aplicación de `safe-area-inset-bottom` que levantaba controles y dejaba una franja visual incorrecta en iPhone. Una captura física del iPhone 13 Pro Max confirmó una franja negra adicional de casi dos filas bajo los campos; la vista de video ahora ancla el último bloque a 8 px del borde visual real, sin volver a sumar el `safe-area` que el contenedor de pantalla ya incorpora. El indicador de la franja de datos muestra `Serie` y el número actual destacado, sin la palabra `trabajo`.
- La barra de la sesión activa quedó organizada en seis controles separados: Ajustes, Anterior, Pausar/Reanudar tiempo del entrenamiento, Lista/Video, Siguiente e Información. Se eliminaron el botón flotante de cambio de vista y el botón flotante de pausa sobre el video; tocar el video controla su reproducción directamente.
