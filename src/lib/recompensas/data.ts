...
export type RecompensasAlumnoVip = {
  disponible: boolean;
  error: string | null;
  saldo: number;
  catalogo: RecompensaVip[];
  canjes: CanjeVip[];
};

export type RecompensasAdminVip = Omit<RecompensasAlumnoVip, "saldo">;
...
export async function obtenerRecompensasAlumnoVip(alumnoId: string): Promise<RecompensasAlumnoVip> {
  const db = createAdminClient();
  const [{ data: catalogo, error: errorCatalogo }, { data: canjes, error: errorCanjes }, saldo] = await Promise.all([
  ...
  if (faltaMigracion(errorCatalogo) || faltaMigracion(errorCanjes)) return { disponible: false, error: null, saldo, catalogo: [], canjes: [] };
  if (errorCatalogo || errorCanjes) return { disponible: false, error: "No pudimos cargar el catálogo o tus canjes.", saldo, catalogo: [], canjes: [] };
...
export async function obtenerAdminRecompensasVip(): Promise<RecompensasAdminVip> {
  const db = createAdminClient();
  const [{ data: catalogo, error: errorCatalogo }, { data: canjes, error: errorCanjes }] = await Promise.all([
  ...
  if (faltaMigracion(errorCatalogo) || faltaMigracion(errorCanjes)) return { disponible: false, error: null, catalogo: [], canjes: [] };
  if (errorCatalogo || errorCanjes) return { disponible: false, error: "No pudimos cargar el catálogo o las solicitudes.", catalogo: [], canjes: [] };
