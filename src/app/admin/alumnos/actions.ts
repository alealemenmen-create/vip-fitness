...
import { reabrirSesionCore, reiniciarPlanCore } from "@/lib/entrenamiento/reiniciar";
...
export async function reiniciarPlanAlumno(_prevState: FormState, formData: FormData): Promise<FormState> {
  ...
  const resultado = await reiniciarPlanCore(alumnoId, { userId: sesion.userId, nombre: sesion.nombre, quien: "el entrenador" });
  if (!resultado.ok) return fail(resultado.error ?? "No se pudo reiniciar el plan.");

  revalidatePath(`/admin/alumnos/${alumnoId}`);
  revalidatePath(`/control-vip/alumnos/${alumnoId}`);
  revalidatePath("/alumno", "layout");
  revalidatePath("/portal-v2", "layout");
  return okState;
}
...
/**
 * Reabre una sesión que quedó completada/registrada por error. Ver
 * `reabrirSesionCore` en `lib/entrenamiento/reiniciar.ts` para el detalle:
 * no borra nada. Si algún día hace falta borrar de verdad una sesión
 * completa (no solo reabrirla), esa es la función `aprobarBorradoSesion` de
 * `admin/borrados/actions.ts`, deliberadamente más destructiva y por eso
 * reservada a un pedido explícito del alumno.
 */
export async function reiniciarSesionAlumno(_prevState: FormState, formData: FormData): Promise<FormState> {
  const sesionAdmin = await requireRol(["entrenador", "admin"]);
  const alumnoId = String(formData.get("alumno_id") || "");
  const sesionId = String(formData.get("sesion_id") || "");
  if (!alumnoId || !sesionId) return fail("Faltan datos de la sesión.");

  const resultado = await reabrirSesionCore(alumnoId, sesionId, {
    userId: sesionAdmin.userId,
    nombre: sesionAdmin.nombre,
    quien: "el entrenador",
  });
  if (!resultado.ok) return fail(resultado.error ?? "No se pudo reabrir la sesión.");
