...
/** Misma pantalla que `/admin/noticias`, mismos componentes y datos. */
export default async function ControlVipV2NoticiasPage({
  searchParams,
}: {
  ...
  return (
    <div className="space-y-6 pb-8">
      ...
      <section className="grid grid-cols-2 gap-3 lg:grid-cols-3" aria-label="Resumen de noticias">
        <AdminStatCard href="/control-vip/noticias#noticias-publicadas" icon={<Megaphone size={20} />} value={anuncios.length} label="Publicadas" detail="Ver noticias visibles" color="#22c55e" />
        <AdminStatCard href="/control-vip/noticias#borradores-noticias" icon={<FileClock size={20} />} value={borradores.length} label="Borradores" detail="Revisar antes de publicar" color="#f59e0b" />
        <AdminStatCard href="/control-vip/alumnos" icon={<Users size={20} />} value="Todos" label="Audiencia" detail="Ver alumnos del portal" color="#3b82f6" />
      </section>
