...
/** Misma pantalla que `/admin/torneos`, mismos componentes y datos. */
export default async function ControlVipV2TorneosPage({
  searchParams,
}: {
  ...
  return (
    <div className="space-y-6 pb-8">
      ...
      <section className="grid grid-cols-2 gap-3 lg:grid-cols-3" aria-label="Resumen de Arena VIP">
        <AdminStatCard href="/control-vip/torneos#torneos-activos" icon={<Trophy size={20} />} value={activos.length} label="En curso" detail="Ver competencias activas" color="#f59e0b" />
        <AdminStatCard href="/control-vip/torneos#torneos-finalizados" icon={<Archive size={20} />} value={cerrados.length} label="Finalizadas" detail="Abrir historial de retos" color="#a78bfa" />
        <AdminStatCard href="/control-vip/alumnos" icon={<Users size={20} />} value={alumnos.length} label="Participantes" detail="Ver alumnos disponibles" color="#3b82f6" />
      </section>
