...
/**
 * Galería en Control VIP V2 (Fase 4, docs/PROYECTO_CONTROL_VIP_V2.md).
 * Mismo componente y mismos datos que `/admin/ejercicios` — `GaleriaEjercicios`
 * ya organiza el trabajo en Pendientes, Mesa, Biblioteca, Carga masiva y
 * Calidad (el instructivo de ingestión multimedia, INSTRUCTIVO_CLAUDE_
 * GALERIA_MULTIMEDIA_CARGA_MASIVA.md, ya está implementado completo desde el
 * 16 de agosto — commit 1cfc22a). Esta pantalla no toca el motor de carga,
 * solo lo presenta dentro del shell nuevo.
 */
export default async function ControlVipV2GaleriaPage() {
  ...
  return (
    <div className="space-y-6 pb-8">
      ...
      <section className="grid grid-cols-2 gap-3 lg:grid-cols-3" aria-label="Resumen de ejercicios">
        <AdminStatCard href="/control-vip/galeria#biblioteca-ejercicios" icon={<Dumbbell size={20} />} value={biblioteca.length} label="Ejercicios" detail="Ver biblioteca disponible" color="#3b82f6" />
        <AdminStatCard href="/control-vip/galeria#inventario-ejercicios" icon={<Images size={20} />} value={`${conFoto} · ${biblioteca.length - conFoto}`} label="Con foto · Sin foto" detail="Ver inventario alfabético" color="#a78bfa" />
        <AdminStatCard href="/control-vip/galeria#reportes-ejercicios" icon={<CircleAlert size={20} />} value={reportes.length} label="Reportes" detail="Revisar imágenes señaladas" color={reportes.length ? "#f59e0b" : "#22c55e"} />
      </section>
